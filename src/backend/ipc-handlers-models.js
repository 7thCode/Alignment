...
class IPCHandlersModels {
  ...
  registerModelHandlers() {
    ...
    // LLMプロンプト実行
    ipcMain.handle("model:prompt", async (event, { prompt, options }) => {
      try {
        const response = await this.llamaManager.prompt(prompt, options);
        return { success: true, response };
      } catch (error) {
        console.error("model:prompt error:", error);
        throw error;
      }
    });
  }
...
module.exports = IPCHandlersModels;
