...
async function loadModelsDirectory() {
  try {
    const result = await window.electronAPI.modelsDirGet();
...
async function loadModels() {
  try {
    presetModels = await window.electronAPI.modelPresets();
    installedModels = await window.electronAPI.modelList();
    renderPresets();
    renderInstalled();
  } catch (error) {
...
async function downloadModel(model) {
  try {
    ...
    renderPresets();
    await window.electronAPI.modelDownload(model);
  } catch (error) {
...
async function changeModelsDirectory() {
  try {
    // ディレクトリ選択ダイアログを表示
    const result = await window.electronAPI.modelsDirSelect();
    ...
    // ディレクトリを設定
    await window.electronAPI.modelsDirSet(newDir);
